 ...
 *
 */
class PerimeterDefenceParser extends AbstractVcaParser {

    public PerimeterDefenceParser(String uniqueName, Map<String, String> defaultParams) {
    ...
    protected Map<String, String> getUserParams(Map thresholdsMap) throws Exception {
        ...
        String centroidRadius = String.format("%1$.0f", thresholdsMap.get("centroidRadius"));
        String maskPercent = String.format("%1$.1f", thresholdsMap.get("maskPercent"));
        String maskInfo = String.format("fg=%s,img=%s", maskPercent, jpegFile);

        Map<String, String> userParams = new LinkedHashMap();
        userParams.put("mask", maskInfo);
        userParams.put("centriodradius", centroidRadius);
