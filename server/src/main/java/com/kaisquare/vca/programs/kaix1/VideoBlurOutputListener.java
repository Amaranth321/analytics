 ...
 * @since v4.5
 */
class VideoBlurOutputListener extends CmdOutputListener
{
    public VideoBlurOutputListener(VcaInfo vcaInfo)
    {
        super(vcaInfo);
    }
    ...
    {
        if (!eventInfoMap.get("evt").equals("detectblur"))
        {
            return;
        }
        ...
        ReportDataEvent.queue(time, vcaInfo.getCamera(), new NoDataReport(vcaInfo.getInstanceId(), EventType.VCA_VIDEO_BLUR));
    }
}
